...
def short_url():
    ...
    if response.status_code == 200:
        ...
        if data.get('status') == 'ok':
            ...
            state_pdf = str(crpdf.get())
            if state_pdf == 'True':
                pdf = FPDF('P', 'mm', 'A4')
                pdf.add_page()
                pdf.set_font('Arial', 'u', 13)
                pdf.cell(0, 0, short_url)
                pdf.link(x=0, y=0, w=210, h=20, link=short_url)
                pdf.output(datetime.datetime.now().strftime("%d.%m.%Y.pdf"))
...
pdfcheck = tk.Checkbutton(root, text='PDF', variable=crpdf, onvalue=1, offvalue=0)
pdfcheck.pack()
